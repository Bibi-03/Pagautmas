...
function PagoTelefonico() {
    ...
    const [data, setData] = useState({monto: 12312, proveedor: proveedores[id-1], servicio: 'Telefonica'})
    
    useEffect(() => {
      const Verificar = async () => {
        try {
          const response = await getVerificarNumCuentaRequest(numCuenta, 3, id);
          setData({monto: response.data.Tasa, proveedor: proveedores[id-1], servicio: 'Telefonica'})
        } catch (error) {
          setPagar(false)
        }
    ...
    const handleClickNext = (e) => {
      e.preventDefault()
      navigate(`/Servicios/Telefonica/${id}/${nCuenta}`)
    }
    ...
    const HandleClickPagar = async (e) => {
      e.preventDefault()
      try {
        await RealizarPagoRequest({numCuenta, Proveedor: id, Servicio: 3, emailUser: currentUser.response.email, monto: data.monto})
        notify()
        await sleep(5000);
        navigate('/Inicio')
      } catch (error) {
