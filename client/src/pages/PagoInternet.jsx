...
function PagoInternet() {
    ...
    const [data, setData] = useState({monto: 12312, proveedor: proveedores[id-1], servicio: 'Internet'})
    
    useEffect(() => {
      const Verificar = async () => {
        try {
          const response = await getVerificarNumCuentaRequest(numCuenta, 2, id);
          setData({monto: response.data.Tasa, proveedor: proveedores[id-1], servicio: 'Internet'})
        } catch (error) {
          setPagar(false)
        }
    ...
    const handleClickNext = (e) => {
      e.preventDefault()
      navigate(`/Servicios/Internet/${id}/${nCuenta}`)
    }
    ...
    const HandleClickPagar = async (e) => {
      e.preventDefault()
      try {
        await RealizarPagoRequest({numCuenta, Proveedor: id, Servicio: 2, emailUser: currentUser.response.email, monto: data.monto})
        notify()
        await sleep(5000);
        navigate('/Inicio')
      } catch (error) {
